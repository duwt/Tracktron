...
class MotChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            'GT_LOC_FORMAT': '{gt_folder}/{seq}/gt/gt_{split_mode}.txt',  # '{gt_folder}/{seq}/gt/gt_{split_mode}.txt'
            'SKIP_SPLIT_FOL': False,  # If False, data is in GT_FOLDER/BENCHMARK-SPLIT_TO_EVAL/ and in
                                      # TRACKERS_FOLDER/BENCHMARK-SPLIT_TO_EVAL/tracker/
                                      # If True, then the middle 'benchmark-split' folder is skipped for both.
            'SPLIT_MODE': 'full',  # Valid: 'half0', 'half1', 'full'
        }
    ...
    def __init__(self, config=None):
        ...
        for seq in self.seq_list:
            if not self.data_is_zipped:
                curr_file = self.config["GT_LOC_FORMAT"].format(gt_folder=self.gt_fol, seq=seq, split_mode=self.split_mode)
        ...
        if self.config["SEQ_INFO"]:
            ...
            for seq, seq_length in seq_lengths.items():
                if seq_length is None:
                    ini_file = os.path.join(self.gt_fol, seq, 'seqinfo_{}.ini'.format(self.split_mode))
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        if self.data_is_zipped:
            ...
            zip_file = None
            if is_gt:
                file = self.config["GT_LOC_FORMAT"].format(gt_folder=self.gt_fol, seq=seq, split_mode=self.split_mode)
            else:
        ...
        start_frame = min(map(int, list(read_data.keys())))
        current_time_keys = [str(t + start_frame) for t in range(num_timesteps)]
