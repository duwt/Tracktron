...
class Evaluator:
    ...
    def evaluate(self, dataset_list, metrics_list):
        ...
        for dataset, dataset_name in zip(dataset_list, dataset_names):
            ...
            logger.info('Evaluating %i tracker(s) on %i sequence(s) for %i class(es) on %s dataset using the following '
                  'metrics: %s' % (len(tracker_list), len(seq_list), len(class_list), dataset_name,
                                     ', '.join(metric_names)))
            ...
            for tracker in tracker_list:
                ...
                try:
                    ...
                    logger.error('Tracker %s was unable to be evaluated.' % tracker)
                    logger.error(err)
                    traceback.print_exc()
