...
class _BaseMetric(ABC):
    ...
    def print_table(self, table_res, tracker, cls):
        ...
        rows_str = ["", self._row_print([metric_name + ': ' + tracker + '-' + cls] + self.summary_fields)]
        for seq, results in sorted(table_res.items()):
            if seq == 'COMBINED_SEQ':
                continue
            summary_res = self._summary_row(results)
            rows_str.append(self._row_print([seq] + summary_res))
        summary_res = self._summary_row(table_res['COMBINED_SEQ'])
        rows_str.append(self._row_print(['COMBINED'] + summary_res))
        logger.info("\n".join(rows_str))
